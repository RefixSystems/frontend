...
const QuotesDetails = () => {
  ...
  const role = getRole();
  const { data: QuotesDetailsData, isLoading } = useGetQuotesDetailsQuery({
    requestId,
    role,
  });
  ...
  useEffect(() => {
    if (QuotesDetailsData && QuotesDetailsData.data) {
      setData(QuotesDetailsData.data);
      setRead(QuotesDetailsData.moduleAccess.read);
    }
  }, [QuotesDetailsData, role]);

  const handleCancel = () => navigate('/admin/quotes');
            ...
            <Container fluid className="mt-3 reduced-width-row">
              <Row className="boxShadow p-4 mb-4 mt-4">
                <Col className="d-flex justify-content-start mb-3 mt-3">
                  ...
                  <h4 className="ms-2">Quotes Details</h4>
                </Col>
              </Row>
                        ...
                        <Row>
                          <Col md={6}>
                            ...
                            <Card.Text>
                              ...
                              <span
                                className={`badge ${item.status === 'Accepted' ? 'bg-success' : 'bg-warning'}`}
                              >
                                {item.status}
                              </span>
                            </Card.Text>
                          ...
                          <Col md={6}>
                            ...
                            <Card.Text>
                              <strong>Purpose Of Rental:</strong>{' '}
                              {item.purposeOfRental}
                            </Card.Text>
                            <Card.Text>
