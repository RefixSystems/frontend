...
import { useGetRefurbishedOrderDetailsQuery } from '../../../redux/api/QuotesDetailsApi';
...
const RefurbishedOrderDetails = () => {
  const { requestId } = useParams();
  const [data, setData] = useState([]);
  const [read, setRead] = useState(false);
  const role = getRole();
  const { data: RefurbishedOrderDetailsData, isLoading } =
    useGetRefurbishedOrderDetailsQuery({ requestId, role });
  const formatDateTime = (date) => format(new Date(date), 'dd-MMM-yyyy h:mm a');
  const navigate = useNavigate();

  useEffect(() => {
    if (RefurbishedOrderDetailsData && RefurbishedOrderDetailsData.data) {
      setData(RefurbishedOrderDetailsData.data);
      setRead(RefurbishedOrderDetailsData.moduleAccess.read);
    }
  }, [RefurbishedOrderDetailsData, role]);
...
export default RefurbishedOrderDetails;
